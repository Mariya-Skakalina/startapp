...
class WorkWithToken:
    ...
    def _create_token(self, jwt_user: Union[User, dict], time: int) -> str:
    ...
    def write_token_in_cookies(self, user) -> HttpResponseRedirect:
        access_token = self._create_token(jwt_user=user, time=1)
        refresh_token = self._create_token(jwt_user=user, time=1000)
    ...
    def update_custom_token(self, user: dict, token: str) -> str:
        access_token = self._create_token(jwt_user=user, time=30)
